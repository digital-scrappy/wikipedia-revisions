...
$core_k(x)$ using the =core distance= we can define the =mutual reachability distance= as: $d_{mreach-k}(a,b) = max \begin{Bmatrix}core_k(a)\\ core_k(b)\\ d(a,b)\end{Bmatrix}$ test
